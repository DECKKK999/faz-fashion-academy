...
// "100_certificate_codes_...xlsx" dan "Certifikate Code.xlsm").
//   A: No.  B: Nama Peserta  C: Certificate ID  D: Nama Kelas  E: Nama Mentor  F: Tanggal
const HEADER_ROW = 1;
const FIRST_DATA_ROW = 2;
const COL = { no: "A", recipient: "B", code: "C", courseTitle: "D", instructor: "E", issuedAt: "F" };
...
export async function fetchAllCertificateRows(): Promise<SheetCertificateRow[]> {
  ...
  const res = await sheets.spreadsheets.values.get({
    ...
    range: tabRange(`A${FIRST_DATA_ROW}:F`),
  });
...
export async function upsertCertificateRowInSheet(row: {
  ...
  await sheets.spreadsheets.values.append({
    spreadsheetId: env.CERTIFICATE_SHEET_ID,
    range: tabRange(`A${HEADER_ROW}:F${HEADER_ROW}`),
    valueInputOption: "USER_ENTERED",
    insertDataOption: "INSERT_ROWS",
    requestBody: {
      values: [[nextNo, row.recipientName, row.code, row.courseTitle, row.instructorName ?? "", row.issuedAtLabel]],
    },
  });
}
